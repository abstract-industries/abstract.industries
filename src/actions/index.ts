  ...
  signup: defineAction({
    ...
    handler: async (input, context) => {
      try {
        ...
        await rateLimiter.enforceRateLimit(email, RATE_LIMIT_CONFIGS.SIGNUP_EMAIL);
        await rateLimiter.enforceRateLimit(clientIP, RATE_LIMIT_CONFIGS.SIGNUP_IP);
  ...
  contact: defineAction({
    ...
    handler: async (input, context) => {
      try {
        ...
        // Apply rate limiting for both email and IP
        await rateLimiter.enforceRateLimit(email, RATE_LIMIT_CONFIGS.CONTACT_EMAIL);
        await rateLimiter.enforceRateLimit(clientIP, RATE_LIMIT_CONFIGS.CONTACT_IP);
        ...
        await CONTACT_STORE.put(emailKey, JSON.stringify({
          contactId,
          timestamp
        }));
