...
export class RateLimiter {
  ...
  async checkRateLimit(
    ...
    if (lastSubmissionData) {
      ...
      const timeDifferenceMinutes = (currentTime.getTime() - lastSubmissionTime.getTime()) / (1000 * 60);

      if (timeDifferenceMinutes < config.windowMinutes) {
        const remainingMinutes = Math.ceil(config.windowMinutes - timeDifferenceMinutes);
